...
def startup_event() -> None:
    ...
    try:
        ...
        triton_url = f"localhost:{TRITON_HTTP_PORT}"

        # Reuse already running Triton (e.g., started by Docker entrypoint) if available.
        try:
            _triton_client = create_triton_client(triton_url)
            logger.info("Connected to existing Triton server at %s", triton_url)
            return
        except Exception:
            logger.info("No live Triton detected at %s; starting local server.", triton_url)
